...
export default function VerifyEmailScreen() {
  ...
  async function onResend() {
    setBusy(true);
    try {
      const newCode = await resendVerificationEmail(); 
      Alert.alert(
        'Sent',
        `We re-sent the verification email.\n\nDEV code: ${newCode ?? 'check logs'}`
      );
    } catch (e: any) {
  ...
  async function onSubmitCode() {
    ...
    try {
      ...
      // Firestore `verified: true` is already set inside verifyWithCode,
      // so AuthProvider will see it and Root will switch to AppDrawer.
  ...
  async function onIUsedEmailLink() {
    ...
    try {
      ...
      if (user.emailVerified) {
        // Mark as verified in Firestore so AuthProvider updates `verified`
        await updateDoc(doc(db, 'users', user.uid), { verified: true });

        // Once this field changes, the AuthProvider's onSnapshot will set
        // verified = true and Root will render <AppDrawer /> instead of this screen.
        Alert.alert('Verified', 'Your email is verified!');
      } else {
        Alert.alert(
          'Not verified yet',
          'Tap the link in your email, then come back and try again.'
        );
      }
    } catch (e: any) {
      Alert.alert('Error', e?.message || 'Could not refresh verification status.');
    } finally {
      setBusy(false);
    }
  }
